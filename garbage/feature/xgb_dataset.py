...
def make_dataset(train_path, test_path, holiday_path):
    ...
    str_col = ['day_of_week', 'start_turn_restricted', 'end_turn_restricted', 'group_time', 'season', 'vacation', 'road_name_new', 'weight_restricted_new']
    ...
    test = test[test['maximum_speed_limit']!=40]
    
    
    X = train.drop(
    ['id', 'base_date', 'target', 'road_name', 'start_node_name', 'end_node_name', 'vehicle_restricted', 'post_date', 'pre_date', 'height_restricted',
     'post_holiday', 'week', 'holiday', 'pre_holiday'], axis=1
    )

    y = train['target']

    test = test.drop(
        ['id', 'base_date', 'road_name', 'start_node_name', 'end_node_name', 'vehicle_restricted', 'post_date', 'pre_date', 'height_restricted',
         'post_holiday', 'week', 'holiday', 'pre_holiday'], axis=1
    )

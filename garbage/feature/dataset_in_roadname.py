...
def make_dataset(train_path, test_path):
    ...
    str_col = ['day_of_week', 'start_turn_restricted', 'end_turn_restricted', 'time', 'season', 'vacation', 'road_name']
    ...
    X = train.drop(
    ['id', 'base_date', 'target', 'start_node_name', 'end_node_name', 'vehicle_restricted', 'base_hour', 'post_date', 'pre_date'], axis=1
    )

    y = train['target']

    test = test.drop(
        ['id', 'base_date', 'start_node_name', 'end_node_name', 'vehicle_restricted', 'base_hour', 'post_date', 'pre_date'], axis=1
    )

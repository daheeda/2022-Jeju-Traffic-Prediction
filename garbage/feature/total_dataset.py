...
def make_dataset(train_path, test_path, holiday_path):    
    ...
    str_col = ['day_of_week', 'start_turn_restricted', 'end_turn_restricted', 'group_time', 'season', 'vacation',
               'road_name_new', 'weight_restricted_new', 'day_of_week_2', 'base_hour_2', 'interval', 'start_cartesian', 'end_cartesian']
    ...
    X = train.drop(
    ['id', 'base_date', 'target', 'road_name', 'start_node_name', 'end_node_name', 'vehicle_restricted', 'base_hour', 'post_date', 'pre_date', 'height_restricted'], axis=1
    )

    y = train['target']

    test = test.drop(
        ['id', 'base_date', 'road_name', 'start_node_name', 'end_node_name', 'vehicle_restricted', 'base_hour', 'post_date', 'pre_date', 'height_restricted'], axis=1
    )
